...
export function getFeedbackComposerPosition(
  ...
  const left = Math.max(
    viewportMargin,
    Math.min(preferredRight <= maximumLeft ? preferredRight : preferredLeft, maximumLeft)
  );
  const maximumTop = Math.max(viewportMargin, viewportHeight - composerHeight - viewportMargin);
  const top = Math.max(viewportMargin, Math.min(target.rect.y, maximumTop));
...
export function FeedbackMode() {
  ...
  const composerPosition = selectedTarget
    ? getFeedbackComposerPosition(selectedTarget, window.innerWidth, window.innerHeight)
    : null;
  ...
  return (
    <>
      ...
      {!capturing && selectedTarget && composerPosition && (
        <section
          data-feedback-ui="true"
          role="dialog"
          aria-label="Add feedback"
          className="fixed z-[9999] w-[340px] overflow-hidden rounded-xl border border-border-primary bg-background-primary shadow-2xl"
          style={composerPosition}
        >
          <header className="flex items-center justify-between gap-3 border-b border-border-primary px-3 py-2.5">
            ...
            <button
              type="button"
              className="rounded p-1 text-text-secondary hover:bg-background-secondary hover:text-text-primary"
              onClick={() => {
                setSelectedTarget(null);
                setComment('');
              }}
          ...
          <div className="p-3">
            <textarea
              ...
              className="min-h-24 w-full resize-none bg-transparent text-sm text-text-primary outline-none placeholder:text-text-secondary"
            />
            <div className="mt-2 flex items-center justify-between border-t border-border-primary pt-2">
              <span className="text-[11px] text-text-secondary">⌘↵ to save</span>
              <button
                type="button"
